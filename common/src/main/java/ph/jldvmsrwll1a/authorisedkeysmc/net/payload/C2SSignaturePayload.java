...
public final class C2SSignaturePayload extends BaseC2SPayload {
    ...
    public static C2SSignaturePayload fromSigningChallenge(Ed25519PrivateKeyParameters signingKey, S2CChallengePayload challenge, byte[] sessionHash, boolean forRegistration) {
    ...
    public boolean verify(Ed25519PublicKeyParameters verifyingKey, byte[] nonce, byte[] sessionHash, boolean forRegistration) {
        byte[] marker = forRegistration ? MARKER_REGISTRATION : MARKER_LOGIN;

        Validate.isTrue(nonce.length == S2CChallengePayload.NONCE_LENGTH, "Nonce must be %s bytes.", C2SChallengePayload.NONCE_LENGTH);
