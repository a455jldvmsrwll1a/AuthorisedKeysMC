...
public final class C2SSignaturePayload extends BaseC2SPayload {
    private static final int SIGNATURE_LENGTH = AkSigner.SIGNATURE_LENGTH;
    ...
    public static C2SSignaturePayload fromSigningChallenge(
            AkPrivateKey signingKey, S2CChallengePayload challenge, byte[] sessionHash, boolean forRegistration) {
        ...
        signer.update(marker);
        signer.update(challenge.getNonce(), S2CChallengePayload.NONCE_LENGTH);

        return new C2SSignaturePayload(signer.sign());
    }
    ...
    public boolean verify(AkPublicKey verifyingKey, byte[] nonce, byte[] sessionHash, boolean forRegistration) {
        byte[] marker = forRegistration ? MARKER_REGISTRATION : MARKER_LOGIN;
        ...
        verifier.update(marker);
        verifier.update(nonce, S2CChallengePayload.NONCE_LENGTH);
